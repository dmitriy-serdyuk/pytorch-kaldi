    ...
    def __init__(self, options):
        ...
        for i in range(self.N_hid):
            ...
            self.wx[i].weight = torch.nn.Parameter(
                torch.Tensor(self.hidden_dim, curr_dim).uniform_(-np.sqrt(0.01 / (curr_dim + self.hidden_dim)),
...
class GRU(nn.Module):
    ...
    def forward(self, x, lab, test_flag):
        ...
        if test_flag == 0:
            drop_mask = Variable(
                torch.bernoulli(torch.Tensor(h_init.shape[0], h_init.shape[1]).fill_(1 - self.drop_rate)))
        else:
